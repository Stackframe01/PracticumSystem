...
def get_specializations():
    ...
    regex = ''
    with open('{}/data/vocabularies/specializations_regex.csv'.format(os.path.dirname(os.path.abspath(__file__)))) as f_in:
        for line in f_in:
...
def get_requirements(vacs):
    regex_start = ''
    with open('{}/data/vocabularies/description_regex_start.csv'.format(os.path.dirname(os.path.abspath(__file__)))) as f_in:
        for line in f_in:
            regex_start += '{}|'.format(line.rstrip())

    regex_end = ''
    with open('{}/data/vocabularies/description_regex_end.csv'.format(os.path.dirname(os.path.abspath(__file__)))) as f_in:
        for line in f_in:
...
def to_csv(file_name, data):
    with open('{}/data/{}.csv'.format(os.path.dirname(os.path.abspath(__file__)), file_name), 'w') as f_out:
        f_out.write('Number;Required skill\n')
